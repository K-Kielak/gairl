...
def main():
    ...
    with tf.Session() as sess:
        gan = create_gan(GAN_STR, imgs.shape[1:], sess,
                         cond_in_size=LABELS_NUM,
        ...
        for t in range(TRAINING_STEPS):
            ...
            gan.train_step(batch_imgs, g_condition=onehot_labels)
            if t % GAN_VIS_FREQ == 0:
                labels_to_vis = np.eye(LABELS_NUM)
                gen_images = gan.generate(10, g_condition=labels_to_vis)
